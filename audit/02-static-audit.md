...
_Stage 2. Defects findable by reading, each survived an independent adversarial falsification pass. Generated 2026-06-17T16:43:34.008Z._

Judged against the Stage-1 provisional intent:
...
- Audit surface (source/test/doc/config): **26**
- Visitation-evidenced: **26**
- Unverified (no tool evidence): **0**

## Surviving findings (58)
...
| F050 | high | bug | `run-task-master.bat:7-8` | run-task-master.bat hardcodes developer-specific username 'Shadow' |
...
| F051 | medium | doc-drift | `run-task-master.bat:1-8; docs/onboarding-guide.md:112-115; docs/task-master-windows-guide.md:65-72` | Three mutually inconsistent documented versions of run-task-master.bat |
| F052 | medium | doc-drift | `docs/onboarding-guide.md:102; docs/task-master-guide.md:59; docs/task-master-windows-guide.md:50,320` | Invalid Claude model ID 'claude-3-5-sonnet-20240229' in all TaskMaster .env examples |
| F053 | medium | design-defect | `.gitignore:199` | .gitignore excludes scripts/ directory, preventing PRD from being version-controlled |
| F056 | medium | doc-drift | `docs/onboarding-guide.md:147,150,153,157,165,169,183,207,211,346,397,404,409` | docs/onboarding-guide.md uses Windows-only .bat syntax throughout a cross-platform setup guide |
...
| F054 | low | design-defect | `.gitignore:201` | .gitignore excludes .cursor directory, preventing sharing of Cursor workflow rules |
| F055 | low | doc-drift | `rebound_readme.md:115` | rebound_readme.md Paper 9 arXiv link is truncated and broken |
| F057 | low | doc-drift | `docs/onboarding-guide.md:72-80` | docs/onboarding-guide.md clone+upstream setup creates redundant remotes without a fork |
| F058 | low | doc-drift | `docs/task-master-windows-guide.md:333` | docs/task-master-windows-guide.md file structure shows tasks.json at root with comment claiming it is inside tasks/ |
...
- **Falsifier (upheld):** simulation_runner.py:84 reads `pbh_mass = pbh_params['mass']`; generate_pbh_sample (parameter_sampler.py:112-117) emits 'mass_msun' not 'mass'. ensemble_runner.py:220 passes pbh_samples[i] verbatim through run_parallel_simulations (line 72) to run_single_simulation, so line 84 raises KeyError, caught by the broad except at simulation_runner.py:180-184 returning (None,None,None); ensemble_runner.py:75-76 then raises RuntimeError. End-to-end ensemble cannot produce a perturbed result. Confirmed.
...
- **Falsifier (upheld):** run_ensemble ends at ensemble_runner.py:277 with a print and no `return results`, despite the docstring promising a list of summary dicts. At line 460 `ensemble_results = run_ensemble(...)` receives None, and line 472 `sum(1 for r in ensemble_results if r.get('status') == 'completed')` iterates None → TypeError.
...
- **Falsifier (upheld):** parameter_sampler.py:113 emits key 'mass_msun' but simulation_runner.py:84 reads `pbh_params['mass']` → KeyError; simulation_runner.py:91 uses `.get('impact_param', 100.0)` while the sampler key is 'impact_param_au' (parameter_sampler.py:114). ensemble_runner.py:72 passes the sampler dict straight through with no key mapping, so every perturbed member crashes at line 84 (caught by the try/except at line 123).
...
- **Falsifier (upheld):** src/parameter_sampler.py:11 reads `KM_S_TO_AU_DAY = 1.0 / 1.731e6 * 86400.0` = 86400/1.731e6 ≈ 0.0499 AU/day per km/s. Correct value is 86400/1.496e8 ≈ 5.78e-4. The divisor 1.731e6 is ~86× smaller than 1.496e8 (the AU-in-km figure even named in the same line's comment), so sampled velocities are ~86× too large. Confirmed.
...
- **Falsifier (upheld):** run_ensemble (ensemble_runner.py:238) creates multiprocessing.Pool and maps run_ensemble_member; pool workers are daemonic by default. run_ensemble_member (line 64) calls simulation_runner.run_parallel_simulations, which at simulation_runner.py:228 unconditionally creates a second multiprocessing.Pool whenever pbh_params is not None. Ensemble always passes pbh_params (line 72), so the perturbed path always reaches line 228. Daemon processes cannot spawn children -> 'daemonic processes are not allowed to have children' AssertionError, caught by the member try/except and recorded as failed. Every member fails. Upheld.
...
- **Falsifier (upheld):** residual_analysis.py:215 is `metadata = eval(metadata_str)` with comment 'Use eval carefully! Assumes trusted source.', where metadata_str derives from data['metadata'].item() (line 214) loaded from the .npz, and save_residuals writes it as a stringified dict at line 148. A crafted .npz metadata array's string is eval'd, giving arbitrary code execution from a data file. Confirmed at the anchor.
...
- **Falsifier (upheld):** parameter_sampler.py:11 is `KM_S_TO_AU_DAY = 1.0 / 1.731e6 * 86400.0` = 0.0499. Correct: 1 km/s = 86400/1.496e8 = 5.78e-4 AU/day, so the constant is ~86x too large. sample_velocity (lines 65-67) multiplies every sampled component by this factor. The comment itself cites 1.496e8 km/AU yet the code uses 1.731e6. Confirmed.
...
- **Falsifier (upheld):** simulation_runner.py:91 is `pbh_initial_pos = np.array([-1000.0, pbh_params.get('impact_param', 100.0), 0.0])`. The sampler emits 'impact_param_au' (parameter_sampler.py:114), never 'impact_param', so .get always returns the hardcoded 100.0. (Note: in the ensemble path execution never reaches line 91 because line 84 KeyErrors first, but the key-mismatch defect at the cited anchor is real.) Confirmed.
...
- **Falsifier (upheld):** simulation_runner.py:104 hardcodes `target_body_label = "body_3"`, while labels are built as f"body_{i}" with index 0 renamed 'Sun' (lines 48-50). For the 3-body example/ensemble setup labels are ['Sun','body_1','body_2'] — no 'body_3'. apply_analytic_kick cannot find the target and returns False (n_body_simulation.py:281-285), so no kick is applied. Confirmed at the anchor.
...
- **Falsifier (upheld):** run_ensemble (ensemble_runner.py:144) docstring (line 172) promises 'list of summary dictionaries', but the body ends at the print on line 277 with no return statement (line 282 begins a new function), so it returns None. The __main__ block assigns `ensemble_results = run_ensemble(...)` at line 460 and iterates `sum(1 for r in ensemble_results ...)` at line 472, raising TypeError on NoneType. Confirmed.
...
- **Falsifier (upheld):** simulation_runner.py:89-91 contains the literal placeholder comments and sets `pbh_initial_pos = np.array([-1000.0, pbh_params.get('impact_param', 100.0), 0.0])` with '# Needs proper calculation!'. The sampler's 't_encounter_years' (parameter_sampler.py:116) is never read in this path; apply_analytic_kick computes t_ca purely from the placed-geometry state (n_body_simulation.py:234-244).
...
- **Falsifier (upheld):** src/analytic_impulse.py:104-105: comment claims '200 km/s -> 94.8 AU/day ... ~= 5510 AU/(yr/2pi)' and hardcodes `pbh_velocity_example = np.array([5510.0, 0, 0])`. Correct: 200 km/s = 200*86400/1.496e8 ≈ 0.1155 AU/day, not 94.8 (~820× off); 5510 = 94.8 * 365.25/(2π). Both comment and value are wrong/relativistic. Confirmed.
...
- **Falsifier (upheld):** src/n_body_simulation.py:4 has unconditional `from analytic_impulse import calculate_velocity_kick, G as analytic_G` with no try/except. tests/test_n_body_simulation.py:13 imports `from src.n_body_simulation import NBodySimulation`; conftest.py:8 adds only the project root (not src/) to sys.path, so the bare top-level `analytic_impulse` lookup fails -> ModuleNotFoundError. Core claim confirmed. Minor inaccuracy in evidence: simulation_runner.py:4-5 actually uses pure relative imports (`from . import ...`), not try/except; this does not affect the finding's substance.
...
- **Falsifier (upheld):** src/simulation_runner.py:88-91 confirmed: TODO comment plus `pbh_initial_pos = np.array([-1000.0, pbh_params.get('impact_param', 100.0), 0.0]) # Needs proper calculation!`. The y-coordinate is set to the impact-parameter scalar regardless of encounter geometry; no spherical-to-cartesian PBH placement exists. Confirmed.
...
- **Falsifier (upheld):** src/simulation_runner.py:104 `target_body_label = "body_3"` with comment assuming Earth is body 3. Labels generated at line 48 as `[f"body_{i}" for i in range(len(masses))]`, so with <4 bodies 'body_3' is absent and the kick silently fails. No argument/param to override the target. Confirmed.
...
- **Falsifier (upheld):** src/residual_analysis.py:233-240 contains only commented-out stubs for `calculate_observables` and `calculate_q_fom` (both `# ... pass`). No q_fom implementation exists; the figure-of-merit detection metric is unimplemented. Confirmed.
...
- **Falsifier (upheld):** README.md:128-131 lists `src/nbody.py`, `src/pbhsampler.py`, `src/residuals.py`, `src/parameter_recovery.py`; actual files are n_body_simulation.py, parameter_sampler.py, residual_analysis.py and there is no parameter_recovery.py. Lines 134-139 list scripts/ files and examples/SingleFlyby.ipynb that do not exist in the audit surface. Confirmed.
...
- **Falsifier (upheld):** README.md:98-113 documents `scripts/single_flyby.py --mass ... --r0 ... --alpha ... --beta ...`, `scripts/ensemble_flyby.py`, `scripts/param_recovery.py`. No scripts/ directory exists in the audit surface; the only example, examples/single_flyby_example.py, is a stub (line 34: 'This is a placeholder example.'). The --r0/--alpha/--beta geometry flags match no implemented interface. Confirmed.
...
- **Falsifier (upheld):** examples/single_flyby_example.py:34 prints the placeholder text verbatim; the entire run_single_flyby_example() body (lines 18-39) is only print() calls and parameter literals, with no import of or call to any src/ simulation module. README.md:115 directs users to examples/ for a guided walkthrough. Confirmed stub.
...
- **Falsifier (upheld):** generate_pbh_sample (parameter_sampler.py:85-120) builds dicts with exactly mass_msun, impact_param_au, velocity_au_day, t_encounter_years (lines 112-117). No sampling of r0/theta0/phi0/alpha/beta. docs/pseudocode.md:201-219 specifies CreatePBHBody requires r0, theta0, phi0, alpha, beta to set the PBH's initial position. The placeholder at simulation_runner.py:90-91 ('TODO: Calculate initial PBH position', dummy hardcoded position) confirms the geometry cannot be derived. Upheld.
...
- **Falsifier (upheld):** ensemble_runner.py:6 is `from tqdm import tqdm` — a bare top-level import (comment '# Optional' notwithstanding, there is no try/except). tqdm is used at line 241. requirements.txt lists only rebound/numpy/scipy/matplotlib/jupyter/emcee/dynesty (no tqdm); setup.py install_requires lists numpy/matplotlib/scipy/rebound/pytest/pytest-cov (no tqdm). A fresh install from declared deps raises ModuleNotFoundError on import of ensemble_runner. Upheld.
...
- **Falsifier (upheld):** simulation_runner.py:94-99 adds the PBH as a live REBOUND particle with full pbh_mass. apply_analytic_kick (n_body_simulation.py:198-298) integrates the full system from t_start to t_ca via integrate_to_time (line 272) — during which the live PBH gravitationally attracts all bodies — then adds the full analytic delta_v to the target velocity (lines 291-293), where delta_v from calculate_velocity_kick is the total impulse of a complete flyby. After apply_analytic_kick returns, the main loop (simulation_runner.py:143-175) keeps integrating to t_end with the PBH still present at full mass. The PBH effect is thus counted three times: live N-body on approach, the full analytic kick, and live N-body on departure. The impulse approximation requires the PBH exert no live forces. Upheld.
...
- **Falsifier (upheld):** Confirmed at simulation_runner.py. Lines 67-75 fill positions_out[0]/velocities_out[0] with the t_start state for len(masses) particles. When pbh_params is truthy, lines 126-127 reassign positions_out/velocities_out to fresh np.zeros of shape (n_steps+1, len(masses)+1, 3), discarding row 0. Lines 130-133 set times_out[0]=sim_instance.get_simulation_time() and fill positions_out[0] with the current state. apply_analytic_kick (n_body_simulation.py:272) integrates the sim to t_apply (=t_ca) before returning, so the current time is t_ca, not t_start. Thus the perturbed output array begins at t_ca and the [t_start, t_ca] pre-encounter state is permanently lost; compute_residuals (residual_analysis.py:98) np.interp then clips for query times below t_ca.
...
- **Falsifier (upheld):** Confirmed at ensemble_runner.py. Line 367: total_completed += 1 runs unconditionally for every member with status=='completed'. Line 368 calls is_detected, which returns None when stats are missing/invalid (lines 299,303,309,313). Line 370-372: 'if detected is None: continue' skips the member from total_detected and from bin counts without decrementing total_completed. Line 394 computes overall_rate = total_detected/total_completed, so unclassifiable members inflate the denominator. Separately, members whose mass falls outside mass_bins (line 385 bound check 0<=bin_index<num_bins) are counted in total_completed but added to no totals_by_bin, making overall and summed-binned rates inconsistent.

### F050 — run-task-master.bat hardcodes developer-specific username 'Shadow'

- **Severity / class:** high / bug
- **Location:** `run-task-master.bat:7-8`
- **Evidence:** Lines 7-8 embed 'C:\Users\Shadow\AppData\Roaming\npm' twice. Every developer whose Windows username is not 'Shadow' will get a path-not-found error on every Task Master command. The documented template in docs/task-master-windows-guide.md:68-72 correctly uses the portable %USERNAME% environment variable, confirming this is a developer-specific artifact that was inadvertently committed.
- **Violates intent:** Breaks the TaskMaster collaboration workflow (onboarding-guide.md Section 1 'TaskMaster Setup') for all contributors except the original developer.
- **Falsifier (upheld):** Verified: run-task-master.bat:7 (`set PATH=%PATH%;C:\Program Files\nodejs;C:\Users\Shadow\AppData\Roaming\npm`) and :8 (`C:\Users\Shadow\AppData\Roaming\npm\task-master.cmd %*`) both hardcode username 'Shadow'. The portable template using %USERNAME% exists at docs/task-master-windows-guide.md:69-70 (finding cites 68-72, header inclusive). A non-'Shadow' user gets a path-not-found. Claim accurate.
...
- **Falsifier (upheld):** residual_analysis.py:206 is `data = np.load(filepath, allow_pickle=True)` with comment 'Allow pickle for metadata potentially', on a caller-supplied filepath. allow_pickle=True permits pickle-based object deserialization, a known RCE vector, compounding F001 on the same load path. Confirmed at the anchor.
...
- **Falsifier (upheld):** n_body_simulation.py:4 uses the top-level absolute import `from analytic_impulse import calculate_velocity_kick, G as analytic_G`, while simulation_runner.py:4 and ensemble_runner.py:10-13 import it package-relatively (from . import n_body_simulation). Under `python -m src.ensemble_runner` the project root (not src/) is on sys.path, so the bare name 'analytic_impulse' (located in src/) is unresolved and importing n_body_simulation raises ModuleNotFoundError. Confirmed real path-fragility at the anchor.
...
- **Falsifier (upheld):** n_body_simulation.py:97-99 confirms `except Exception as e:` only `print(f"Error adding Solar System bodies: {e}")` with comment 'Consider fallback or raising the error' (line 99). No re-raise, no status return; the method continues and downstream code proceeds regardless of whether bodies were added.
...
- **Falsifier (upheld):** analytic_impulse.py:33 `initial_relative_position = pbh_position - body_position` and line 48 `r_ca = initial_relative_position + relative_velocity * t_ca` both point from body toward PBH. Gravity accelerates the body toward the PBH (+r_ca), but line 66 sets `kick_direction = -r_ca / b` (away from PBH). apply_kick adds delta_v directly (analytic_impulse.py:95 `current_velocity + delta_v`) with no downstream negation, so the wrong-direction impulse is applied as-is.
...
- **Falsifier (upheld):** summary['pbh_params'] is the raw sampler dict (ensemble_runner.py:54) containing numpy ndarray 'velocity_au_day' and numpy float 'mass_msun' (parameter_sampler.py:113,115). json.dump at ensemble_runner.py:136 (per-member), :249 (checkpoint), and :263 (final) cannot serialize numpy types and raises TypeError; each is wrapped in try/except that only prints a warning (lines 137-138, 251-252, 273-274), so these outputs are not persisted.
...
- **Falsifier (upheld):** src/residual_analysis.py:215 calls eval(metadata_str) with the comment 'Use eval carefully! Assumes trusted source.' The string comes from data['metadata'] loaded via np.load(allow_pickle=True) at line 206 — arbitrary Python executes on a crafted .npz. No ast.literal_eval/json.loads alternative present. Confirmed code-injection risk.
...
- **Falsifier (upheld):** src/residual_analysis.py: the success branch (line 363 'if res_t is not None...') handles save/load; the else: at line 394 prints 'Residual computation failed'. The 'Calculate Stats' block (lines 397-410, calculate_residual_stats(res_p, res_v)) is indented at 8 spaces inside that else branch, so it runs only on failure — where res_p/res_v are None — and never on success. Confirmed.
...
- **Falsifier (upheld):** grep confirms unconditional plt.show() at src/visualization.py:76, 175, 314, 384 (one per plot function), with no show= parameter or headless guard. Blocks/errors in batch/server ensemble contexts. Confirmed.
...
- **Falsifier (upheld):** setup.py install_requires (lines 10-17) contains 'pytest' (line 15) and 'pytest-cov' (line 16) — minor off-by-one vs cited line 14 but inside install_requires, not extras_require. Test frameworks forced as runtime deps. Confirmed.
...
- **Falsifier (upheld):** setup.py:9 uses packages=find_packages(); `ls src/__init__.py` returns 'No such file or directory', so src/ is not a discoverable package and only tests/ (which has __init__.py) would be found. name='primordial_encounters' matches no directory. A pip install would omit all simulation code. Confirmed.
...
- **Falsifier (upheld):** README.md:53-56 documents Feature 5 'Parameter Recovery & Statistical Testing' (recover PBH mass, likelihood ratio tests), and README:110-113 references scripts/param_recovery.py. grep for parameter_recovery|RecoverPBH|likelihood across src/ returns no files; src/ listing shows no parameter_recovery module. synthetic_data.py only adds noise. Confirmed doc-drift.
...
- **Falsifier (upheld):** README.md:58-60 documents Feature 6 'Spectral Analysis (Optional)' with Fourier methods. grep for power_spectrum|spectral|fft|periodogram across src/ returns no files. No FFT/spectral module exists in src/. Confirmed doc-drift.
...
- **Falsifier (upheld):** README.md:160 contains 'This project is offered under the [MIT License](LICENSE).' linking to a LICENSE file. `ls` of the repo root shows no LICENSE file (grep -i licen returned nothing), and LICENSE is absent from the Stage-1 surface. The dangling link and missing license declaration are confirmed.
...
- **Falsifier (upheld):** tests/test_n_body_simulation.py:23-27 is `def test_initialization(self): ... pass` (docstring + comment + bare pass). test_project_structure (lines 29-40) only asserts os.path.isdir for src/tests/examples/data. No numerical/physics assertions exist; the only other test file content is directory existence. Confirmed.
...
- **Falsifier (upheld):** Confirmed at residual_analysis.py:98-99: np.interp(residual_times, pert_times, pert_pos[:,particle_idx,dim]) with no overlap/range check. np.interp returns the boundary (first/last) value for query points outside [pert_times[0], pert_times[-1]] rather than raising or NaN-ing. The only guard is line 86 (identical-array fast path), which does not check range coverage. residual_times=base_times (line 81), so when base_times extends below/beyond pert_times the residual is computed against a clipped constant — silently wrong.
...
- **Falsifier (upheld):** Confirmed at visualization.py. Line 127 builds valid_indices = [idx for idx in particle_indices if 0 <= idx < n_particles_res] — values bounded by the residuals array size, i.e. residuals-array positions. Line 132 selects residuals[:, valid_indices, :] (residuals-local indexing) and line 137 reuses the same valid_indices to index the caller's labels list, which the docstring (lines 96-98) documents as indexed by *original* particle indices. The __main__ example (lines 442-451) passes res_p_eg (columns = original particles 1 and 2 per lines 431-437), particle_indices=[0,1], labels=['Sun','Earth','PlanetX'] (labels_eg), so plot_labels becomes [labels[0],labels[1]]=['Sun','Earth'] instead of the correct ['Earth','PlanetX']. Mislabeling confirmed.
...
- **Falsifier (upheld):** Confirmed at ensemble_runner.py:316: peak_residual_magnitude = np.linalg.norm(pos_peak_au[target_particle_idx, :]). pos_peak_au comes from calculate_peak (residual_analysis.py:293: np.max(np.abs(data_residuals), axis=0)), so its three entries are per-dimension temporal maxima taken independently (generally at different time steps). sqrt(max_x^2+max_y^2+max_z^2) >= max_t sqrt(x(t)^2+y(t)^2+z(t)^2) since each per-dim max bounds that dim's value at every t; the norm therefore overestimates the true peak 3D displacement and can make peak_residual_magnitude > threshold_au (line 318) when no instantaneous 3D displacement ever exceeded the threshold, yielding false-positive detections. (The 'Cauchy-Schwarz' attribution is loose, but the inequality direction is correct.)
...
- **Falsifier (upheld):** Confirmed by direct inspection. Glob for **/__init__.py returns only tests/__init__.py; no src/__init__.py exists. setup.py:9 calls find_packages() with no where= argument, so it scans the project root; without src/__init__.py, 'src' is not detected as a regular package and pip install . would install nothing from src/. Relative imports requiring package context are present: simulation_runner.py:4-5 (from . import n_body_simulation/analytic_impulse), ensemble_runner.py:10-13 (from . import parameter_sampler/...), synthetic_data.py:6 (from . import residual_analysis). This is a distinct, independent cause from a directory-name/find_packages mismatch. Minor inaccuracy in the finding's supporting detail: visualization.py does NOT use a try/except relative import; it uses an absolute 'from ensemble_runner import calculate_detection_rates' at line 510 inside __main__. This does not undermine the core design defect, which is upheld.

### F051 — Three mutually inconsistent documented versions of run-task-master.bat

- **Severity / class:** medium / doc-drift
- **Location:** `run-task-master.bat:1-8; docs/onboarding-guide.md:112-115; docs/task-master-windows-guide.md:65-72`
- **Evidence:** onboarding-guide.md:113-115 shows '@echo off / npx task-master-ai %*' (portable, no path manipulation). task-master-windows-guide.md:68-72 shows a three-line file using '%USERNAME%' for portable npm paths. The committed run-task-master.bat uses a five-line file with hardcoded 'Shadow' username. None of the three versions matches any other.
- **Violates intent:** New contributors following either documentation source will produce a file that differs from what is committed, undermining reproducibility of the development environment.
- **Falsifier (upheld):** Three inconsistent versions confirmed: committed run-task-master.bat:1-8 (node-check + hardcoded 'Shadow' set PATH + task-master.cmd); onboarding-guide.md:113-114 (`@echo off` / `npx task-master-ai %*`); task-master-windows-guide.md:68-70 (`@echo off` / set PATH with %USERNAME% / task-master.cmd %USERNAME%). None matches another. Minor inaccuracy: finding calls the committed file 'five-line' but it is 8 content lines; does not affect the three-way mismatch claim.

### F052 — Invalid Claude model ID 'claude-3-5-sonnet-20240229' in all TaskMaster .env examples

- **Severity / class:** medium / doc-drift
- **Location:** `docs/onboarding-guide.md:102; docs/task-master-guide.md:59; docs/task-master-windows-guide.md:50,320`
- **Evidence:** All three docs specify MODEL=claude-3-5-sonnet-20240229. Claude 3.5 Sonnet launched in June 2024 with id claude-3-5-sonnet-20240620; no Anthropic model carries the date suffix 20240229. task-master-windows-guide.md:319-322 additionally lists this invalid ID in a 'current valid models' enumeration alongside the valid claude-3-opus-20240229 and claude-3-haiku-20240307. Developers copying this value verbatim will receive 401/404 API errors from Anthropic.
- **Violates intent:** Blocks TaskMaster AI features (task generation, expansion, complexity analysis) for every new contributor until they manually discover and fix the model ID.
- **Falsifier (upheld):** Core defect verified: invalid ID claude-3-5-sonnet-20240229 (3.5-Sonnet never carried the 20240229 date; real IDs are -20240620/-20241022, while 20240229 belongs to claude-3-opus/claude-3-sonnet) appears in copyable .env examples at onboarding-guide.md:101 (finding cited 102, off-by-1) and task-master-windows-guide.md:52 (finding cited 50, off-by-2), and in the 'current valid models' list at task-master-windows-guide.md:320. HOWEVER the finding's claim that all three docs and specifically task-master-guide.md:59 specify this ID is REFUTED: line 59 shows `MODEL: ... (e.g., "claude-3-7-sonnet-20250219")`, a valid model, not the invalid one. Underlying defect holds; the 'all three docs' / tm-guide:59 attribution is wrong.

### F053 — .gitignore excludes scripts/ directory, preventing PRD from being version-controlled

- **Severity / class:** medium / design-defect
- **Location:** `.gitignore:199`
- **Evidence:** Line 199 lists 'scripts/' as an ignored path. docs/task-master-guide.md:107 instructs users to run 'task-master parse-prd scripts/prd.txt'. The PRD file that seeds the entire task database cannot be committed to the repository; every developer must obtain or recreate it out-of-band.
- **Violates intent:** Undermines the TaskMaster-driven workflow that the project explicitly adopts: without a committed PRD, reproducible task generation is impossible for new contributors.
- **Falsifier (upheld):** Verified: .gitignore:199 is `scripts/`. task-master-guide.md:107 instructs `task-master parse-prd scripts/prd.txt`. No negation (!scripts/prd.txt) present. The PRD seed file is therefore excluded from version control. Claim accurate.

### F056 — docs/onboarding-guide.md uses Windows-only .bat syntax throughout a cross-platform setup guide

- **Severity / class:** medium / doc-drift
- **Location:** `docs/onboarding-guide.md:147,150,153,157,165,169,183,207,211,346,397,404,409`
- **Evidence:** All TaskMaster commands in the guide use '.\run-task-master.bat' with a Windows backslash and .bat extension. The same guide includes Linux/macOS venv activation instructions (lines 129-131), establishing cross-platform intent. No Unix equivalent (run-task-master.sh or 'npx task-master-ai') is provided. On Linux/macOS, '.bat' files do not execute natively.
- **Violates intent:** Linux/macOS contributors — the likely majority in an astronomy/astrophysics research context — cannot follow any TaskMaster step in the onboarding guide without ad-hoc adaptation.
- **Falsifier (upheld):** Verified: onboarding-guide.md uses `.\run-task-master.bat` at 147,152,157,164,169,182,208,213,396,404,447 (finding's specific line list drifts by 1-2 on several but every cited region resolves to a .bat invocation). Cross-platform intent established by macOS/Linux venv activation at :131 (`source venv/bin/activate`). No run-task-master.sh exists (only run-task-master.bat present); no npx fallback given. .bat does not run natively on Linux/macOS. Claim accurate.
...
- **Falsifier (upheld):** simulation_runner.py:125-127 allocates arrays to len(masses)+1, yet the loop at lines 151-161 re-checks particle count and reallocates/copies with the comment 'this is inefficient, better to size correctly initially'. The min-shape copy (158-161) drops data on shrink and the bounds check (165-169) silently skips out-of-range particles, masking sizing errors rather than failing.
...
- **Falsifier (upheld):** examples/single_flyby_example.py docstring (lines 4-8) advertises set-up/run/analyze, but the body (lines 20-39) only prints parameters then `print("\nThis is a placeholder example. The actual implementation will:")` (line 34). It imports no src module and runs no simulation.
...
- **Falsifier (upheld):** src/parameter_sampler.py:2 is `import scipy.stats as stats`. Reading the full module (lines 1-162), no `stats.` reference appears; all sampling uses np.random.uniform/np.random.normal (lines 25, 41, 60-62, 83). The import is unused; scipy is a declared dependency (requirements.txt:4, setup.py:13).
...
- **Falsifier (upheld):** residual_analysis.py:6 defines FORMAT_CSV='csv'. save_residuals CSV branch (lines 154-177) prints 'CSV saving is not yet implemented' and returns False. Core finding confirmed. Minor inaccuracy in the parenthetical: the signature default is FORMAT_NPZ, not CSV (line 114: format=FORMAT_NPZ); FORMAT_CSV is a public module constant but not the default. This does not undermine the doc-drift finding.
...
- **Falsifier (upheld):** Confirmed. Line 38: dt_rebound = dt_years * 2 * np.pi if dt_years else None — a truthy test that maps dt_years=0.0 to None. Line 65 separately falls back to n_steps=100 for falsy dt_years, so the while loop at line 143 (current_step < n_steps) is entered, and line 144 evaluates min(sim_instance.sim.t + dt_rebound, ...) which raises TypeError (float + NoneType) when dt_rebound is None. The error is caught by the broad except at line 180 (returns None,None,None), but the reachable defect path the finding describes is real and dt_years is documented (line 21) as only used 'if needed'.
...
- **Falsifier (upheld):** Confirmed. visualization.py:510 inside __main__: 'from ensemble_runner import calculate_detection_rates' (bare, non-package import); its ImportError is silently caught at line 528 ('Skipping binned rate plot example'). synthetic_data.py:156 inside __main__ try block: 'import visualization' (bare); its ImportError is silently caught at line 174 ('Skipping comparison plot'). Both are bare absolute imports unresolvable under package-mode (python -m src.x) execution, matching the flagged n_body_simulation.py class of defect, at two additional locations with swallowed errors.

### F054 — .gitignore excludes .cursor directory, preventing sharing of Cursor workflow rules

- **Severity / class:** low / design-defect
- **Location:** `.gitignore:201`
- **Evidence:** Line 201 ignores '.cursor'. docs/task-master-guide.md:101 states 'The task-master init command creates a .cursor/rules/dev_workflow.mdc file. Cursor automatically loads this file, providing the AI agent with context about Task Master commands and workflow.' With .cursor gitignored, this rules file is never committed and each developer must regenerate it individually.
- **Violates intent:** Each new Cursor-based contributor loses the pre-configured AI workflow context that the guide describes as central to the development environment.
- **Falsifier (upheld):** Verified: .gitignore:201 is `.cursor`. task-master-guide.md:101 states `task-master init` creates `.cursor/rules/dev_workflow.mdc`. With .cursor ignored the rules file is never committed. Claim accurate.

### F055 — rebound_readme.md Paper 9 arXiv link is truncated and broken

- **Severity / class:** low / doc-drift
- **Location:** `rebound_readme.md:115`
- **Evidence:** Line 115 reads '<https://ui.adsabs.harvard.edu/abs/>' with no abstract identifier appended. All other paper links in the same file include a full ADS identifier (e.g. 'abs/2012A%26A...537A.128R'). This link resolves to a generic ADS search page rather than the specific paper.
- **Violates intent:** Breaks citation traceability for REBOUND Paper 9 (high-order symplectic integrators), which is relevant reference material for the N-body simulation pipeline.
- **Falsifier (upheld):** Verified: rebound_readme.md:115 (Paper 9, Rein, Tamayo & Brown 2019) ends with `<https://ui.adsabs.harvard.edu/abs/>` with no ADS identifier appended, whereas peers (e.g. :111 `abs/2018MNRAS.473.3351R`) carry full identifiers. Link is truncated/broken. Claim accurate.

### F057 — docs/onboarding-guide.md clone+upstream setup creates redundant remotes without a fork

- **Severity / class:** low / doc-drift
- **Location:** `docs/onboarding-guide.md:72-80`
- **Evidence:** Line 73-74 clones the main repo URL (setting origin to it). Lines 79-80 immediately add 'upstream' pointing to the identical URL. A fork-based workflow requires forking the repo first so origin points to the fork and upstream to the canonical source. Without forking, both remotes resolve to the same repository, making the upstream remote meaningless and misleading contributors about the intended contribution model.
- **Violates intent:** Misleads contributors into a non-functional fork workflow; direct pushes to origin would go to the main repo, not a personal fork.
- **Falsifier (upheld):** Verified: onboarding-guide.md:73 clones https://github.com/ImmortalDemonGod/PrimordialEncounters.git (origin) and :79 adds upstream pointing to the identical URL, with no fork step. Both remotes resolve to the same repo, making upstream redundant/misleading for a fork-based workflow. Claim accurate.

### F058 — docs/task-master-windows-guide.md file structure shows tasks.json at root with comment claiming it is inside tasks/

- **Severity / class:** low / doc-drift
- **Location:** `docs/task-master-windows-guide.md:333`
- **Evidence:** Line 333 shows '├── tasks.json   # Task data (in tasks/ directory)' — the tree path places it at the project root but the inline comment claims it lives inside tasks/. The .gitignore ignores 'tasks.json' at root (line 197) and 'tasks/' directory (line 198), so both locations are excluded from version control regardless. The contradiction causes confusion about actual task data location.
- **Violates intent:** Minor but confusing for contributors trying to understand where task state is persisted.
- **Falsifier (upheld):** Content verified: task-master-windows-guide.md shows `tasks.json   # Task data (in tasks/ directory)` placed at project-root level in the tree while the inline comment claims it lives inside tasks/ — a contradiction. Both .gitignore:197 (tasks.json) and :198 (tasks/) exclude either location. Minor inaccuracy: the line is 331, not the cited 333 (off-by-2). Substantive claim holds.
...
- **Falsifier (upheld):** Confirmed contradiction in n_body_simulation.py. Docstring lines 202-204: 'Assumes the simulation is at the initial time (t=0) where initial positions/velocities are used for the kick calculation.' Implementation lines 214-217 read initial_time_years = self.get_simulation_time() and comment 'We calculate the kick based on the state at the *current* simulation time.' Line 243 uses self.sim.t as the base for t_ca. The code uses current time, not t=0; the docstring is misleading. doc-drift/info confirmed.

## Refuted / unverified candidates (2)
...
| docs/task-master-guide.md lists 'npx task-master-init' as local-install init command instead of 'npx task-master-ai init' | `docs/task-master-guide.md:46-48` | refuted | no verdict |
...
```json
{
  "findings": [
    {
      ...
      "id": "F001",
      "status": "survived",
      "falsifier_note": "residual_analysis.py:215 is `metadata = eval(metadata_str)` with comment 'Use eval carefully! Assumes trusted source.', where metadata_str derives from data['metadata'].item() (line 214) loaded from the .npz, and save_residuals writes it as a stringified dict at line 148. A crafted .npz metadata array's string is eval'd, giving arbitrary code execution from a data file. Confirmed at the anchor."
    },
    {
      ...
      "id": "F002",
      "status": "survived",
      "falsifier_note": "residual_analysis.py:206 is `data = np.load(filepath, allow_pickle=True)` with comment 'Allow pickle for metadata potentially', on a caller-supplied filepath. allow_pickle=True permits pickle-based object deserialization, a known RCE vector, compounding F001 on the same load path. Confirmed at the anchor."
    },
    {
      ...
      "id": "F003",
      "status": "survived",
      "falsifier_note": "parameter_sampler.py:11 is `KM_S_TO_AU_DAY = 1.0 / 1.731e6 * 86400.0` = 0.0499. Correct: 1 km/s = 86400/1.496e8 = 5.78e-4 AU/day, so the constant is ~86x too large. sample_velocity (lines 65-67) multiplies every sampled component by this factor. The comment itself cites 1.496e8 km/AU yet the code uses 1.731e6. Confirmed."
    },
    {
      ...
      "id": "F004",
      "status": "survived",
      "falsifier_note": "simulation_runner.py:84 reads `pbh_mass = pbh_params['mass']`; generate_pbh_sample (parameter_sampler.py:112-117) emits 'mass_msun' not 'mass'. ensemble_runner.py:220 passes pbh_samples[i] verbatim through run_parallel_simulations (line 72) to run_single_simulation, so line 84 raises KeyError, caught by the broad except at simulation_runner.py:180-184 returning (None,None,None); ensemble_runner.py:75-76 then raises RuntimeError. End-to-end ensemble cannot produce a perturbed result. Confirmed."
    },
    {
      ...
      "id": "F005",
      "status": "survived",
      "falsifier_note": "simulation_runner.py:91 is `pbh_initial_pos = np.array([-1000.0, pbh_params.get('impact_param', 100.0), 0.0])`. The sampler emits 'impact_param_au' (parameter_sampler.py:114), never 'impact_param', so .get always returns the hardcoded 100.0. (Note: in the ensemble path execution never reaches line 91 because line 84 KeyErrors first, but the key-mismatch defect at the cited anchor is real.) Confirmed."
    },
    {
      ...
      "id": "F006",
      "status": "survived",
      "falsifier_note": "simulation_runner.py:104 hardcodes `target_body_label = \"body_3\"`, while labels are built as f\"body_{i}\" with index 0 renamed 'Sun' (lines 48-50). For the 3-body example/ensemble setup labels are ['Sun','body_1','body_2'] — no 'body_3'. apply_analytic_kick cannot find the target and returns False (n_body_simulation.py:281-285), so no kick is applied. Confirmed at the anchor."
    },
    {
      ...
      "id": "F007",
      "status": "survived",
      "falsifier_note": "run_ensemble (ensemble_runner.py:144) docstring (line 172) promises 'list of summary dictionaries', but the body ends at the print on line 277 with no return statement (line 282 begins a new function), so it returns None. The __main__ block assigns `ensemble_results = run_ensemble(...)` at line 460 and iterates `sum(1 for r in ensemble_results ...)` at line 472, raising TypeError on NoneType. Confirmed."
    },
    {
      ...
      "id": "F008",
      "status": "survived",
      "falsifier_note": "n_body_simulation.py:4 uses the top-level absolute import `from analytic_impulse import calculate_velocity_kick, G as analytic_G`, while simulation_runner.py:4 and ensemble_runner.py:10-13 import it package-relatively (from . import n_body_simulation). Under `python -m src.ensemble_runner` the project root (not src/) is on sys.path, so the bare name 'analytic_impulse' (located in src/) is unresolved and importing n_body_simulation raises ModuleNotFoundError. Confirmed real path-fragility at the anchor."
    },
    {
      ...
      "id": "F009",
      "status": "survived",
      "falsifier_note": "n_body_simulation.py:97-99 confirms `except Exception as e:` only `print(f\"Error adding Solar System bodies: {e}\")` with comment 'Consider fallback or raising the error' (line 99). No re-raise, no status return; the method continues and downstream code proceeds regardless of whether bodies were added."
    },
    {
      ...
      "id": "F010",
      "status": "survived",
      "falsifier_note": "simulation_runner.py:89-91 contains the literal placeholder comments and sets `pbh_initial_pos = np.array([-1000.0, pbh_params.get('impact_param', 100.0), 0.0])` with '# Needs proper calculation!'. The sampler's 't_encounter_years' (parameter_sampler.py:116) is never read in this path; apply_analytic_kick computes t_ca purely from the placed-geometry state (n_body_simulation.py:234-244)."
    },
    {
      ...
      "id": "F011",
      "status": "survived",
      "falsifier_note": "analytic_impulse.py:33 `initial_relative_position = pbh_position - body_position` and line 48 `r_ca = initial_relative_position + relative_velocity * t_ca` both point from body toward PBH. Gravity accelerates the body toward the PBH (+r_ca), but line 66 sets `kick_direction = -r_ca / b` (away from PBH). apply_kick adds delta_v directly (analytic_impulse.py:95 `current_velocity + delta_v`) with no downstream negation, so the wrong-direction impulse is applied as-is."
    },
    {
      ...
      "id": "F012",
      "status": "survived",
      "falsifier_note": "summary['pbh_params'] is the raw sampler dict (ensemble_runner.py:54) containing numpy ndarray 'velocity_au_day' and numpy float 'mass_msun' (parameter_sampler.py:113,115). json.dump at ensemble_runner.py:136 (per-member), :249 (checkpoint), and :263 (final) cannot serialize numpy types and raises TypeError; each is wrapped in try/except that only prints a warning (lines 137-138, 251-252, 273-274), so these outputs are not persisted."
    },
    {
      ...
      "id": "F013",
      "status": "survived",
      "falsifier_note": "simulation_runner.py:125-127 allocates arrays to len(masses)+1, yet the loop at lines 151-161 re-checks particle count and reallocates/copies with the comment 'this is inefficient, better to size correctly initially'. The min-shape copy (158-161) drops data on shrink and the bounds check (165-169) silently skips out-of-range particles, masking sizing errors rather than failing."
    },
    {
      ...
      "id": "F014",
      "status": "survived",
      "falsifier_note": "examples/single_flyby_example.py docstring (lines 4-8) advertises set-up/run/analyze, but the body (lines 20-39) only prints parameters then `print(\"\\nThis is a placeholder example. The actual implementation will:\")` (line 34). It imports no src module and runs no simulation."
    },
    {
      ...
      "id": "F015",
      "status": "survived",
      "falsifier_note": "run_ensemble ends at ensemble_runner.py:277 with a print and no `return results`, despite the docstring promising a list of summary dicts. At line 460 `ensemble_results = run_ensemble(...)` receives None, and line 472 `sum(1 for r in ensemble_results if r.get('status') == 'completed')` iterates None → TypeError."
    },
    {
      ...
      "id": "F016",
      "status": "survived",
      "falsifier_note": "parameter_sampler.py:113 emits key 'mass_msun' but simulation_runner.py:84 reads `pbh_params['mass']` → KeyError; simulation_runner.py:91 uses `.get('impact_param', 100.0)` while the sampler key is 'impact_param_au' (parameter_sampler.py:114). ensemble_runner.py:72 passes the sampler dict straight through with no key mapping, so every perturbed member crashes at line 84 (caught by the try/except at line 123)."
    },
    {
      ...
      "id": "F017",
      "status": "survived",
      "falsifier_note": "src/parameter_sampler.py:11 reads `KM_S_TO_AU_DAY = 1.0 / 1.731e6 * 86400.0` = 86400/1.731e6 ≈ 0.0499 AU/day per km/s. Correct value is 86400/1.496e8 ≈ 5.78e-4. The divisor 1.731e6 is ~86× smaller than 1.496e8 (the AU-in-km figure even named in the same line's comment), so sampled velocities are ~86× too large. Confirmed."
    },
    {
      ...
      "id": "F018",
      "status": "survived",
      "falsifier_note": "src/analytic_impulse.py:104-105: comment claims '200 km/s -> 94.8 AU/day ... ~= 5510 AU/(yr/2pi)' and hardcodes `pbh_velocity_example = np.array([5510.0, 0, 0])`. Correct: 200 km/s = 200*86400/1.496e8 ≈ 0.1155 AU/day, not 94.8 (~820× off); 5510 = 94.8 * 365.25/(2π). Both comment and value are wrong/relativistic. Confirmed."
    },
    {
      ...
      "id": "F019",
      "status": "survived",
      "falsifier_note": "src/n_body_simulation.py:4 has unconditional `from analytic_impulse import calculate_velocity_kick, G as analytic_G` with no try/except. tests/test_n_body_simulation.py:13 imports `from src.n_body_simulation import NBodySimulation`; conftest.py:8 adds only the project root (not src/) to sys.path, so the bare top-level `analytic_impulse` lookup fails -> ModuleNotFoundError. Core claim confirmed. Minor inaccuracy in evidence: simulation_runner.py:4-5 actually uses pure relative imports (`from . import ...`), not try/except; this does not affect the finding's substance."
    },
    {
      ...
      "id": "F020",
      "status": "survived",
      "falsifier_note": "src/simulation_runner.py:88-91 confirmed: TODO comment plus `pbh_initial_pos = np.array([-1000.0, pbh_params.get('impact_param', 100.0), 0.0]) # Needs proper calculation!`. The y-coordinate is set to the impact-parameter scalar regardless of encounter geometry; no spherical-to-cartesian PBH placement exists. Confirmed."
    },
    {
      ...
      "id": "F021",
      "status": "survived",
      "falsifier_note": "src/simulation_runner.py:104 `target_body_label = \"body_3\"` with comment assuming Earth is body 3. Labels generated at line 48 as `[f\"body_{i}\" for i in range(len(masses))]`, so with <4 bodies 'body_3' is absent and the kick silently fails. No argument/param to override the target. Confirmed."
    },
    {
      ...
      "id": "F022",
      "status": "survived",
      "falsifier_note": "src/residual_analysis.py:233-240 contains only commented-out stubs for `calculate_observables` and `calculate_q_fom` (both `# ... pass`). No q_fom implementation exists; the figure-of-merit detection metric is unimplemented. Confirmed."
    },
    {
      ...
      "id": "F023",
      "status": "survived",
      "falsifier_note": "README.md:128-131 lists `src/nbody.py`, `src/pbhsampler.py`, `src/residuals.py`, `src/parameter_recovery.py`; actual files are n_body_simulation.py, parameter_sampler.py, residual_analysis.py and there is no parameter_recovery.py. Lines 134-139 list scripts/ files and examples/SingleFlyby.ipynb that do not exist in the audit surface. Confirmed."
    },
    {
      ...
      "id": "F024",
      "status": "survived",
      "falsifier_note": "README.md:98-113 documents `scripts/single_flyby.py --mass ... --r0 ... --alpha ... --beta ...`, `scripts/ensemble_flyby.py`, `scripts/param_recovery.py`. No scripts/ directory exists in the audit surface; the only example, examples/single_flyby_example.py, is a stub (line 34: 'This is a placeholder example.'). The --r0/--alpha/--beta geometry flags match no implemented interface. Confirmed."
    },
    {
    ...
    },
    {
      ...
      "id": "F026",
      "status": "survived",
      "falsifier_note": "src/residual_analysis.py:215 calls eval(metadata_str) with the comment 'Use eval carefully! Assumes trusted source.' The string comes from data['metadata'] loaded via np.load(allow_pickle=True) at line 206 — arbitrary Python executes on a crafted .npz. No ast.literal_eval/json.loads alternative present. Confirmed code-injection risk."
    },
    {
      ...
      "id": "F027",
      "status": "survived",
      "falsifier_note": "src/residual_analysis.py: the success branch (line 363 'if res_t is not None...') handles save/load; the else: at line 394 prints 'Residual computation failed'. The 'Calculate Stats' block (lines 397-410, calculate_residual_stats(res_p, res_v)) is indented at 8 spaces inside that else branch, so it runs only on failure — where res_p/res_v are None — and never on success. Confirmed."
    },
    {
      ...
      "id": "F028",
      "status": "survived",
      "falsifier_note": "grep confirms unconditional plt.show() at src/visualization.py:76, 175, 314, 384 (one per plot function), with no show= parameter or headless guard. Blocks/errors in batch/server ensemble contexts. Confirmed."
    },
    {
      ...
      "id": "F029",
      "status": "survived",
      "falsifier_note": "setup.py install_requires (lines 10-17) contains 'pytest' (line 15) and 'pytest-cov' (line 16) — minor off-by-one vs cited line 14 but inside install_requires, not extras_require. Test frameworks forced as runtime deps. Confirmed."
    },
    {
      ...
      "id": "F030",
      "status": "survived",
      "falsifier_note": "setup.py:9 uses packages=find_packages(); `ls src/__init__.py` returns 'No such file or directory', so src/ is not a discoverable package and only tests/ (which has __init__.py) would be found. name='primordial_encounters' matches no directory. A pip install would omit all simulation code. Confirmed."
    },
    {
      ...
      "id": "F031",
      "status": "survived",
      "falsifier_note": "README.md:53-56 documents Feature 5 'Parameter Recovery & Statistical Testing' (recover PBH mass, likelihood ratio tests), and README:110-113 references scripts/param_recovery.py. grep for parameter_recovery|RecoverPBH|likelihood across src/ returns no files; src/ listing shows no parameter_recovery module. synthetic_data.py only adds noise. Confirmed doc-drift."
    },
    {
      ...
      "id": "F032",
      "status": "survived",
      "falsifier_note": "README.md:58-60 documents Feature 6 'Spectral Analysis (Optional)' with Fourier methods. grep for power_spectrum|spectral|fft|periodogram across src/ returns no files. No FFT/spectral module exists in src/. Confirmed doc-drift."
    },
    {
      ...
      "id": "F033",
      "status": "survived",
      "falsifier_note": "README.md:160 contains 'This project is offered under the [MIT License](LICENSE).' linking to a LICENSE file. `ls` of the repo root shows no LICENSE file (grep -i licen returned nothing), and LICENSE is absent from the Stage-1 surface. The dangling link and missing license declaration are confirmed."
    },
    {
    ...
    },
    {
      ...
      "id": "F035",
      "status": "survived",
      "falsifier_note": "src/parameter_sampler.py:2 is `import scipy.stats as stats`. Reading the full module (lines 1-162), no `stats.` reference appears; all sampling uses np.random.uniform/np.random.normal (lines 25, 41, 60-62, 83). The import is unused; scipy is a declared dependency (requirements.txt:4, setup.py:13)."
    },
    {
      ...
      "id": "F036",
      "status": "survived",
      "falsifier_note": "residual_analysis.py:6 defines FORMAT_CSV='csv'. save_residuals CSV branch (lines 154-177) prints 'CSV saving is not yet implemented' and returns False. Core finding confirmed. Minor inaccuracy in the parenthetical: the signature default is FORMAT_NPZ, not CSV (line 114: format=FORMAT_NPZ); FORMAT_CSV is a public module constant but not the default. This does not undermine the doc-drift finding."
    },
    {
      ...
      "id": "F037",
      "status": "survived",
      "falsifier_note": "generate_pbh_sample (parameter_sampler.py:85-120) builds dicts with exactly mass_msun, impact_param_au, velocity_au_day, t_encounter_years (lines 112-117). No sampling of r0/theta0/phi0/alpha/beta. docs/pseudocode.md:201-219 specifies CreatePBHBody requires r0, theta0, phi0, alpha, beta to set the PBH's initial position. The placeholder at simulation_runner.py:90-91 ('TODO: Calculate initial PBH position', dummy hardcoded position) confirms the geometry cannot be derived. Upheld."
    },
    {
      ...
      "id": "F038",
      "status": "survived",
      "falsifier_note": "run_ensemble (ensemble_runner.py:238) creates multiprocessing.Pool and maps run_ensemble_member; pool workers are daemonic by default. run_ensemble_member (line 64) calls simulation_runner.run_parallel_simulations, which at simulation_runner.py:228 unconditionally creates a second multiprocessing.Pool whenever pbh_params is not None. Ensemble always passes pbh_params (line 72), so the perturbed path always reaches line 228. Daemon processes cannot spawn children -> 'daemonic processes are not allowed to have children' AssertionError, caught by the member try/except and recorded as failed. Every member fails. Upheld."
    },
    {
    ...
    },
    {
      ...
      "id": "F041",
      "status": "survived",
      "falsifier_note": "Confirmed at simulation_runner.py. Lines 67-75 fill positions_out[0]/velocities_out[0] with the t_start state for len(masses) particles. When pbh_params is truthy, lines 126-127 reassign positions_out/velocities_out to fresh np.zeros of shape (n_steps+1, len(masses)+1, 3), discarding row 0. Lines 130-133 set times_out[0]=sim_instance.get_simulation_time() and fill positions_out[0] with the current state. apply_analytic_kick (n_body_simulation.py:272) integrates the sim to t_apply (=t_ca) before returning, so the current time is t_ca, not t_start. Thus the perturbed output array begins at t_ca and the [t_start, t_ca] pre-encounter state is permanently lost; compute_residuals (residual_analysis.py:98) np.interp then clips for query times below t_ca."
    },
    {
      ...
      "id": "F042",
      "status": "survived",
      "falsifier_note": "Confirmed at residual_analysis.py:98-99: np.interp(residual_times, pert_times, pert_pos[:,particle_idx,dim]) with no overlap/range check. np.interp returns the boundary (first/last) value for query points outside [pert_times[0], pert_times[-1]] rather than raising or NaN-ing. The only guard is line 86 (identical-array fast path), which does not check range coverage. residual_times=base_times (line 81), so when base_times extends below/beyond pert_times the residual is computed against a clipped constant — silently wrong."
    },
    {
      ...
      "id": "F043",
      "status": "survived",
      "falsifier_note": "Confirmed. Line 38: dt_rebound = dt_years * 2 * np.pi if dt_years else None — a truthy test that maps dt_years=0.0 to None. Line 65 separately falls back to n_steps=100 for falsy dt_years, so the while loop at line 143 (current_step < n_steps) is entered, and line 144 evaluates min(sim_instance.sim.t + dt_rebound, ...) which raises TypeError (float + NoneType) when dt_rebound is None. The error is caught by the broad except at line 180 (returns None,None,None), but the reachable defect path the finding describes is real and dt_years is documented (line 21) as only used 'if needed'."
    },
    {
      ...
      "id": "F044",
      "status": "survived",
      "falsifier_note": "Confirmed contradiction in n_body_simulation.py. Docstring lines 202-204: 'Assumes the simulation is at the initial time (t=0) where initial positions/velocities are used for the kick calculation.' Implementation lines 214-217 read initial_time_years = self.get_simulation_time() and comment 'We calculate the kick based on the state at the *current* simulation time.' Line 243 uses self.sim.t as the base for t_ca. The code uses current time, not t=0; the docstring is misleading. doc-drift/info confirmed."
    },
    {
      ...
      "id": "F045",
      "status": "survived",
      "falsifier_note": "Confirmed. visualization.py:510 inside __main__: 'from ensemble_runner import calculate_detection_rates' (bare, non-package import); its ImportError is silently caught at line 528 ('Skipping binned rate plot example'). synthetic_data.py:156 inside __main__ try block: 'import visualization' (bare); its ImportError is silently caught at line 174 ('Skipping comparison plot'). Both are bare absolute imports unresolvable under package-mode (python -m src.x) execution, matching the flagged n_body_simulation.py class of defect, at two additional locations with swallowed errors."
    },
    {
      ...
      "id": "F046",
      "status": "survived",
      "falsifier_note": "Confirmed at ensemble_runner.py. Line 367: total_completed += 1 runs unconditionally for every member with status=='completed'. Line 368 calls is_detected, which returns None when stats are missing/invalid (lines 299,303,309,313). Line 370-372: 'if detected is None: continue' skips the member from total_detected and from bin counts without decrementing total_completed. Line 394 computes overall_rate = total_detected/total_completed, so unclassifiable members inflate the denominator. Separately, members whose mass falls outside mass_bins (line 385 bound check 0<=bin_index<num_bins) are counted in total_completed but added to no totals_by_bin, making overall and summed-binned rates inconsistent."
    },
    {
      ...
      "id": "F047",
      "status": "survived",
      "falsifier_note": "Confirmed at visualization.py. Line 127 builds valid_indices = [idx for idx in particle_indices if 0 <= idx < n_particles_res] — values bounded by the residuals array size, i.e. residuals-array positions. Line 132 selects residuals[:, valid_indices, :] (residuals-local indexing) and line 137 reuses the same valid_indices to index the caller's labels list, which the docstring (lines 96-98) documents as indexed by *original* particle indices. The __main__ example (lines 442-451) passes res_p_eg (columns = original particles 1 and 2 per lines 431-437), particle_indices=[0,1], labels=['Sun','Earth','PlanetX'] (labels_eg), so plot_labels becomes [labels[0],labels[1]]=['Sun','Earth'] instead of the correct ['Earth','PlanetX']. Mislabeling confirmed."
    },
    {
      ...
      "id": "F048",
      "status": "survived",
      "falsifier_note": "Confirmed at ensemble_runner.py:316: peak_residual_magnitude = np.linalg.norm(pos_peak_au[target_particle_idx, :]). pos_peak_au comes from calculate_peak (residual_analysis.py:293: np.max(np.abs(data_residuals), axis=0)), so its three entries are per-dimension temporal maxima taken independently (generally at different time steps). sqrt(max_x^2+max_y^2+max_z^2) >= max_t sqrt(x(t)^2+y(t)^2+z(t)^2) since each per-dim max bounds that dim's value at every t; the norm therefore overestimates the true peak 3D displacement and can make peak_residual_magnitude > threshold_au (line 318) when no instantaneous 3D displacement ever exceeded the threshold, yielding false-positive detections. (The 'Cauchy-Schwarz' attribution is loose, but the inequality direction is correct.)"
    },
    {
      ...
      "id": "F049",
      "status": "survived",
      "falsifier_note": "Confirmed by direct inspection. Glob for **/__init__.py returns only tests/__init__.py; no src/__init__.py exists. setup.py:9 calls find_packages() with no where= argument, so it scans the project root; without src/__init__.py, 'src' is not detected as a regular package and pip install . would install nothing from src/. Relative imports requiring package context are present: simulation_runner.py:4-5 (from . import n_body_simulation/analytic_impulse), ensemble_runner.py:10-13 (from . import parameter_sampler/...), synthetic_data.py:6 (from . import residual_analysis). This is a distinct, independent cause from a directory-name/find_packages mismatch. Minor inaccuracy in the finding's supporting detail: visualization.py does NOT use a try/except relative import; it uses an absolute 'from ensemble_runner import calculate_detection_rates' at line 510 inside __main__. This does not undermine the core design defect, which is upheld."
    },
    {
      "title": "run-task-master.bat hardcodes developer-specific username 'Shadow'",
      "location": "run-task-master.bat:7-8",
      "class": "bug",
      "severity": "high",
      "evidence": "Lines 7-8 embed 'C:\\Users\\Shadow\\AppData\\Roaming\\npm' twice. Every developer whose Windows username is not 'Shadow' will get a path-not-found error on every Task Master command. The documented template in docs/task-master-windows-guide.md:68-72 correctly uses the portable %USERNAME% environment variable, confirming this is a developer-specific artifact that was inadvertently committed.",
      "intent_link": "Breaks the TaskMaster collaboration workflow (onboarding-guide.md Section 1 'TaskMaster Setup') for all contributors except the original developer.",
      "id": "F050",
      "status": "survived",
      "falsifier_note": "Verified: run-task-master.bat:7 (`set PATH=%PATH%;C:\\Program Files\\nodejs;C:\\Users\\Shadow\\AppData\\Roaming\\npm`) and :8 (`C:\\Users\\Shadow\\AppData\\Roaming\\npm\\task-master.cmd %*`) both hardcode username 'Shadow'. The portable template using %USERNAME% exists at docs/task-master-windows-guide.md:69-70 (finding cites 68-72, header inclusive). A non-'Shadow' user gets a path-not-found. Claim accurate."
    },
    {
      "title": "Three mutually inconsistent documented versions of run-task-master.bat",
      "location": "run-task-master.bat:1-8; docs/onboarding-guide.md:112-115; docs/task-master-windows-guide.md:65-72",
      "class": "doc-drift",
      "severity": "medium",
      "evidence": "onboarding-guide.md:113-115 shows '@echo off / npx task-master-ai %*' (portable, no path manipulation). task-master-windows-guide.md:68-72 shows a three-line file using '%USERNAME%' for portable npm paths. The committed run-task-master.bat uses a five-line file with hardcoded 'Shadow' username. None of the three versions matches any other.",
      "intent_link": "New contributors following either documentation source will produce a file that differs from what is committed, undermining reproducibility of the development environment.",
      "id": "F051",
      "status": "survived",
      "falsifier_note": "Three inconsistent versions confirmed: committed run-task-master.bat:1-8 (node-check + hardcoded 'Shadow' set PATH + task-master.cmd); onboarding-guide.md:113-114 (`@echo off` / `npx task-master-ai %*`); task-master-windows-guide.md:68-70 (`@echo off` / set PATH with %USERNAME% / task-master.cmd %USERNAME%). None matches another. Minor inaccuracy: finding calls the committed file 'five-line' but it is 8 content lines; does not affect the three-way mismatch claim."
    },
    {
      "title": "Invalid Claude model ID 'claude-3-5-sonnet-20240229' in all TaskMaster .env examples",
      "location": "docs/onboarding-guide.md:102; docs/task-master-guide.md:59; docs/task-master-windows-guide.md:50,320",
      "class": "doc-drift",
      "severity": "medium",
      "evidence": "All three docs specify MODEL=claude-3-5-sonnet-20240229. Claude 3.5 Sonnet launched in June 2024 with id claude-3-5-sonnet-20240620; no Anthropic model carries the date suffix 20240229. task-master-windows-guide.md:319-322 additionally lists this invalid ID in a 'current valid models' enumeration alongside the valid claude-3-opus-20240229 and claude-3-haiku-20240307. Developers copying this value verbatim will receive 401/404 API errors from Anthropic.",
      "intent_link": "Blocks TaskMaster AI features (task generation, expansion, complexity analysis) for every new contributor until they manually discover and fix the model ID.",
      "id": "F052",
      "status": "survived",
      "falsifier_note": "Core defect verified: invalid ID claude-3-5-sonnet-20240229 (3.5-Sonnet never carried the 20240229 date; real IDs are -20240620/-20241022, while 20240229 belongs to claude-3-opus/claude-3-sonnet) appears in copyable .env examples at onboarding-guide.md:101 (finding cited 102, off-by-1) and task-master-windows-guide.md:52 (finding cited 50, off-by-2), and in the 'current valid models' list at task-master-windows-guide.md:320. HOWEVER the finding's claim that all three docs and specifically task-master-guide.md:59 specify this ID is REFUTED: line 59 shows `MODEL: ... (e.g., \"claude-3-7-sonnet-20250219\")`, a valid model, not the invalid one. Underlying defect holds; the 'all three docs' / tm-guide:59 attribution is wrong."
    },
    {
      "title": ".gitignore excludes scripts/ directory, preventing PRD from being version-controlled",
      "location": ".gitignore:199",
      "class": "design-defect",
      "severity": "medium",
      "evidence": "Line 199 lists 'scripts/' as an ignored path. docs/task-master-guide.md:107 instructs users to run 'task-master parse-prd scripts/prd.txt'. The PRD file that seeds the entire task database cannot be committed to the repository; every developer must obtain or recreate it out-of-band.",
      "intent_link": "Undermines the TaskMaster-driven workflow that the project explicitly adopts: without a committed PRD, reproducible task generation is impossible for new contributors.",
      "id": "F053",
      "status": "survived",
      "falsifier_note": "Verified: .gitignore:199 is `scripts/`. task-master-guide.md:107 instructs `task-master parse-prd scripts/prd.txt`. No negation (!scripts/prd.txt) present. The PRD seed file is therefore excluded from version control. Claim accurate."
    },
    {
      "title": ".gitignore excludes .cursor directory, preventing sharing of Cursor workflow rules",
      "location": ".gitignore:201",
      "class": "design-defect",
      "severity": "low",
      "evidence": "Line 201 ignores '.cursor'. docs/task-master-guide.md:101 states 'The task-master init command creates a .cursor/rules/dev_workflow.mdc file. Cursor automatically loads this file, providing the AI agent with context about Task Master commands and workflow.' With .cursor gitignored, this rules file is never committed and each developer must regenerate it individually.",
      "intent_link": "Each new Cursor-based contributor loses the pre-configured AI workflow context that the guide describes as central to the development environment.",
      "id": "F054",
      "status": "survived",
      "falsifier_note": "Verified: .gitignore:201 is `.cursor`. task-master-guide.md:101 states `task-master init` creates `.cursor/rules/dev_workflow.mdc`. With .cursor ignored the rules file is never committed. Claim accurate."
    },
    {
      "title": "rebound_readme.md Paper 9 arXiv link is truncated and broken",
      "location": "rebound_readme.md:115",
      "class": "doc-drift",
      "severity": "low",
      "evidence": "Line 115 reads '<https://ui.adsabs.harvard.edu/abs/>' with no abstract identifier appended. All other paper links in the same file include a full ADS identifier (e.g. 'abs/2012A%26A...537A.128R'). This link resolves to a generic ADS search page rather than the specific paper.",
      "intent_link": "Breaks citation traceability for REBOUND Paper 9 (high-order symplectic integrators), which is relevant reference material for the N-body simulation pipeline.",
      "id": "F055",
      "status": "survived",
      "falsifier_note": "Verified: rebound_readme.md:115 (Paper 9, Rein, Tamayo & Brown 2019) ends with `<https://ui.adsabs.harvard.edu/abs/>` with no ADS identifier appended, whereas peers (e.g. :111 `abs/2018MNRAS.473.3351R`) carry full identifiers. Link is truncated/broken. Claim accurate."
    },
    {
      "title": "docs/onboarding-guide.md uses Windows-only .bat syntax throughout a cross-platform setup guide",
      "location": "docs/onboarding-guide.md:147,150,153,157,165,169,183,207,211,346,397,404,409",
      "class": "doc-drift",
      "severity": "medium",
      "evidence": "All TaskMaster commands in the guide use '.\\run-task-master.bat' with a Windows backslash and .bat extension. The same guide includes Linux/macOS venv activation instructions (lines 129-131), establishing cross-platform intent. No Unix equivalent (run-task-master.sh or 'npx task-master-ai') is provided. On Linux/macOS, '.bat' files do not execute natively.",
      "intent_link": "Linux/macOS contributors — the likely majority in an astronomy/astrophysics research context — cannot follow any TaskMaster step in the onboarding guide without ad-hoc adaptation.",
      "id": "F056",
      "status": "survived",
      "falsifier_note": "Verified: onboarding-guide.md uses `.\\run-task-master.bat` at 147,152,157,164,169,182,208,213,396,404,447 (finding's specific line list drifts by 1-2 on several but every cited region resolves to a .bat invocation). Cross-platform intent established by macOS/Linux venv activation at :131 (`source venv/bin/activate`). No run-task-master.sh exists (only run-task-master.bat present); no npx fallback given. .bat does not run natively on Linux/macOS. Claim accurate."
    },
    {
      "title": "docs/onboarding-guide.md clone+upstream setup creates redundant remotes without a fork",
      "location": "docs/onboarding-guide.md:72-80",
      "class": "doc-drift",
      "severity": "low",
      "evidence": "Line 73-74 clones the main repo URL (setting origin to it). Lines 79-80 immediately add 'upstream' pointing to the identical URL. A fork-based workflow requires forking the repo first so origin points to the fork and upstream to the canonical source. Without forking, both remotes resolve to the same repository, making the upstream remote meaningless and misleading contributors about the intended contribution model.",
      "intent_link": "Misleads contributors into a non-functional fork workflow; direct pushes to origin would go to the main repo, not a personal fork.",
      "id": "F057",
      "status": "survived",
      "falsifier_note": "Verified: onboarding-guide.md:73 clones https://github.com/ImmortalDemonGod/PrimordialEncounters.git (origin) and :79 adds upstream pointing to the identical URL, with no fork step. Both remotes resolve to the same repo, making upstream redundant/misleading for a fork-based workflow. Claim accurate."
    },
    {
      "title": "docs/task-master-windows-guide.md file structure shows tasks.json at root with comment claiming it is inside tasks/",
      "location": "docs/task-master-windows-guide.md:333",
      "class": "doc-drift",
      "severity": "low",
      "evidence": "Line 333 shows '├── tasks.json   # Task data (in tasks/ directory)' — the tree path places it at the project root but the inline comment claims it lives inside tasks/. The .gitignore ignores 'tasks.json' at root (line 197) and 'tasks/' directory (line 198), so both locations are excluded from version control regardless. The contradiction causes confusion about actual task data location.",
      "intent_link": "Minor but confusing for contributors trying to understand where task state is persisted.",
      "id": "F058",
      "status": "survived",
      "falsifier_note": "Content verified: task-master-windows-guide.md shows `tasks.json   # Task data (in tasks/ directory)` placed at project-root level in the tree while the inline comment claims it lives inside tasks/ — a contradiction. Both .gitignore:197 (tasks.json) and :198 (tasks/) exclude either location. Minor inaccuracy: the line is 331, not the cited 333 (off-by-2). Substantive claim holds."
    }
  ],
  "refuted_or_unverified": [
    ...
    {
      "title": "docs/task-master-guide.md lists 'npx task-master-init' as local-install init command instead of 'npx task-master-ai init'",
      "location": "docs/task-master-guide.md:46-48",
      "class": "doc-drift",
      "severity": "medium",
      "evidence": "Lines 46-48 state 'If installed locally: npx task-master-init'. The npm package name is 'task-master-ai' (lines 21-22 in the same file). 'npx task-master-init' would attempt to execute a separate package called 'task-master-init' which does not exist on npm. The global form on line 40 correctly uses 'task-master init' (space-separated subcommand). The local form should be 'npx task-master-ai init'.",
      "intent_link": "Local-installation users following this doc will get an npm package-not-found error when trying to initialize Task Master.",
      "id": "F010",
      "status": "refuted",
      "falsifier_note": "no verdict"
    }
  ],
  "coverage": {
    "surface_count": 26,
